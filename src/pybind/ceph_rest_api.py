...
def load_conf(clustername='ceph', conffile=None):
    ...
    raise Exception('No conf file found for "{0}"'.format(clustername))
...
def api_setup():
    ...
    clientname = os.environ.get('CEPH_RESTAPI_CLIENT_NAME', DEFAULT_CLIENTNAME)
    try:
        err = ''
        ...
        err = str(e)
    if cfg is None:
        err = "Can't load Ceph conf file: " + err
